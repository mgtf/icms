...
/* If you declare any globals in php_icms.h uncomment this:
ZEND_DECLARE_MODULE_GLOBALS(icms)
*/
...
/* {{{ icms_functions[]
 *
 * Every user visible function must have an entry in icms_functions[].
 */
function_entry icms_functions[] = {
	PHP_FE(icms_tpl_find_end,	NULL)
	PHP_FE(icms_tpl_find_char,	NULL)
	{NULL, NULL, NULL}	/* Must be the last line in icms_functions[] */
};
/* }}} */
...
PHP_MINFO_FUNCTION(icms)
{
	...
	php_info_print_table_header(2, "iota content management system support (icms)", "enabled");
